/* This file was generated by SableCC (http://www.sablecc.org/). */

package minijava.node;

import minijava.analysis.*;

@SuppressWarnings("nls")
public final class AMinusExp extends PExp
{
    private PExp _lhs_;
    private PExp _rhs_;
    private TTokMinus _token_;

    public AMinusExp()
    {
        // Constructor
    }

    public AMinusExp(
        @SuppressWarnings("hiding") PExp _lhs_,
        @SuppressWarnings("hiding") PExp _rhs_,
        @SuppressWarnings("hiding") TTokMinus _token_)
    {
        // Constructor
        setLhs(_lhs_);

        setRhs(_rhs_);

        setToken(_token_);

    }

    @Override
    public Object clone()
    {
        return new AMinusExp(
            cloneNode(this._lhs_),
            cloneNode(this._rhs_),
            cloneNode(this._token_));
    }

    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAMinusExp(this);
    }

    public PExp getLhs()
    {
        return this._lhs_;
    }

    public void setLhs(PExp node)
    {
        if(this._lhs_ != null)
        {
            this._lhs_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._lhs_ = node;
    }

    public PExp getRhs()
    {
        return this._rhs_;
    }

    public void setRhs(PExp node)
    {
        if(this._rhs_ != null)
        {
            this._rhs_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._rhs_ = node;
    }

    public TTokMinus getToken()
    {
        return this._token_;
    }

    public void setToken(TTokMinus node)
    {
        if(this._token_ != null)
        {
            this._token_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._token_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._lhs_)
            + toString(this._rhs_)
            + toString(this._token_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._lhs_ == child)
        {
            this._lhs_ = null;
            return;
        }

        if(this._rhs_ == child)
        {
            this._rhs_ = null;
            return;
        }

        if(this._token_ == child)
        {
            this._token_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._lhs_ == oldChild)
        {
            setLhs((PExp) newChild);
            return;
        }

        if(this._rhs_ == oldChild)
        {
            setRhs((PExp) newChild);
            return;
        }

        if(this._token_ == oldChild)
        {
            setToken((TTokMinus) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
